...
def isRoundWinner(n, x):
    ...
    for i in range(n):
        ...
        for idx, num in enumerate(list):
            # if already picked prime number then
            # find if number is multiple of the prime number
            if prime != -1:
                if num % prime == 0:
            ...
            # else check is num is prime then pick it
                ...
                if isPrime(num):
                    selectedIdxs.append(idx)
                    prime = num
        # if failed to pick then current player has lost
        if prime == -1:
...
def isPrime(n):
    # 0, 1, even numbers greater than 2 NOT PRIME
    if n == 1 or n == 0 or (n % 2 == 0 and n > 2):
        return False
    else:
        # Not prime if divisable by another number less than it
        # or equal to the square root of itself.
        # n**(1/2) is square root of n
